...
int             gro = 0;
int             draw_flg = 0;
float           copy_array_right[];
float           copy_array_left[];
...
int             rotate_flg;
float           alfa = 0.5; // KOKO! (BUFFALO = 2.5)(CCD = 0.5)
...
//        if (gr_count <= 6) {
            ...
            glVertex2d(mak_posx + 6, (ysize - gr_pos) - (ysize / 12)); // KOKO! (BUFFALO = 12)(CCD = 15)
            glVertex2d(mak_posx - 6, (ysize - gr_pos) - (ysize / 12));
            glVertex2d(mak_posx - 6, 0);
            glVertex2d(mak_posx + 6, 0);
            glEnd();
//        }
        if (gr_count > 6) {
            glBegin(GL_QUADS);
            glColor3f(0.0, 0.0, 0.0);
            glVertex2d(mak_posx + gr[gro], (ysize - gr_pos) - (ysize / 12) + gr_noise[gro]); // KOKO! (BUFFALO = 12)(CCD = 15)
            glVertex2d(mak_posx + gr[gro + 1], (ysize - gr_pos) - (ysize / 12) + gr_noise[gro]);
            glVertex2d(mak_posx + gr[gro + 1], 0);
            glVertex2d(mak_posx + gr[gro], 0);
            glEnd();
            glBegin(GL_QUADS);
            glColor3f(0.0, 0.0, 0.0);
            glVertex2d(mak_posx + gr[gro + 1], (ysize - gr_pos) - (ysize / 12) + gr_noise[gro]); // KOKO! (BUFFALO = 12)(CCD = 15)
            glVertex2d(mak_posx + gr[gro + 2], (ysize - gr_pos) - (ysize / 12) + gr_noise[gro]);
            glVertex2d(mak_posx + gr[gro + 2], 0);
            glVertex2d(mak_posx + gr[gro + 1], 0);
            glEnd();
            glBegin(GL_QUADS);
            glColor3f(0.0, 0.0, 0.0);
            glVertex2d(mak_posx + gr[gro + 2], (ysize - gr_pos) - (ysize / 12) + gr_noise[gro]); // KOKO! (BUFFALO = 12)(CCD = 15)
            glVertex2d(mak_posx + gr[gro + 3], (ysize - gr_pos) - (ysize / 12) + gr_noise[gro]);
            glVertex2d(mak_posx + gr[gro + 3], 0);
            glVertex2d(mak_posx + gr[gro + 2], 0);
            glEnd();
            glBegin(GL_QUADS);
            glColor3f(0.0, 0.0, 0.0);
            glVertex2d(mak_posx + gr[gro + 3], (ysize - gr_pos) - (ysize / 12) + gr_noise[gro]); // KOKO! (BUFFALO = 12)(CCD = 15)
            glVertex2d(mak_posx + gr[gro + 4], (ysize - gr_pos) - (ysize / 12) + gr_noise[gro]);
            glVertex2d(mak_posx + gr[gro + 4], 0);
            glVertex2d(mak_posx + gr[gro + 3], 0);
            glEnd();
        }
        */
...
//        }
        if (gr_count < xsize) {
            gr_count += 6; // KOKO! (BUFFALO = 40)(CCD = 6)
            groot_count++;
        }
        gro++;
        glColor3f(1.0, 1.0, 1.0);
    ...
    for (l = 0; l < SNOW; l++) {
        if (test_flag == 1 && snowxpos_flg[l] == 0) {
            ...
            snow_noise[l] = GetRandom(-1.0, 1.0);
            snowxpos_flg[l] = 1;
        }
        ...
        glPushMatrix();
            if (rotate_flg == 2) {
                if (cosx_plus >= 0.995 && cosx_plus < 1.0) {
                    xflow[l] += 0;
                }
                else if (cosx_plus >= 0.98 && cosx_plus < 0.995) {
                    xflow[l] += alfa / 2;
                }
            ...
            if (rotate_flg == 1) {
                if (cosx_minus >= 0.995 && cosx_minus < 1.0) {
                    xflow[l] -= 0;
                }
                else if (cosx_plus >= 0.98 && cosx_plus < 0.995) {
                    xflow[l] -= (alfa / 2);
                }
                else if (cosx_minus >= 0.97 && cosx_minus < 0.98) {
                    xflow[l] -= alfa;
                }
                else if (cosx_minus >= 0.96 && cosx_minus < 0.97) {
                    xflow[l] -= (alfa * 2);
                }
                else if (cosx_minus >= 0.95 && cosx_minus < 0.96) {
                    xflow[l] -= (alfa * 3);
                }
                else if (cosx_minus >= 0.94 && cosx_minus < 0.95) {
                    xflow[l] -= (alfa * 4);
                }
                else if (cosx_minus >= 0.93 && cosx_minus < 0.94) {
                    xflow[l] -= (alfa * 5);
                }
                else if (cosx_minus >= 0.92 && cosx_minus < 0.93) {
                    xflow[l] -= (alfa * 6);
                }
                else if (cosx_minus >= 0.91 && cosx_minus < 0.92) {
                    xflow[l] -= (alfa * 7);
                }
                else if (cosx_minus >= 0.90 && cosx_minus < 0.91) {
                    xflow[l] -= (alfa * 8);
                }
                else if (cosx_minus >= 0.88 && cosx_minus < 0.89) {
                    xflow[l] -= (alfa * 9);
                }
                else if (cosx_minus >= 0.87 && cosx_minus < 0.88) {
                    xflow[l] -= (alfa * 10);
                }
                else if (cosx_minus >= 0.86 && cosx_minus < 0.87) {
                    xflow[l] -= (alfa * 11);
                }
                else if (cosx_minus >= 0.85 && cosx_minus < 0.86) {
                    xflow[l] -= (alfa * 12);
                }
                else if (cosx_minus >= 0.84 && cosx_minus < 0.85) {
                    xflow[l] -= (alfa * 13);
                }
                else if (cosx_minus >= 0.825 && cosx_minus < 0.84) {
                    xflow[l] -= (alfa * 14);
                }
            ...
            if (((ysize - snow_ypos[l]) - down_count[l]) >= 0) { // KOKO! (BUFFALO = -200)(CCD = 0)
                down_count[l] = down_count[l] + 3 - snow_noise[l]; // KOKO! (BUFFALO = 20)(CCD = 3)
            }
...
float GetRandom_left(float min, float max)
{
//    srand((unsigned)time(NULL));
	return min + (float)((rand()-(rand()%10))*(max-min+1.0)/(1.0+RAND_MAX));
}
float GetRandom_right(float min, float max)
{
//    srand((unsigned)time(NULL));
	return min + (float)((rand()-(rand()%10))*(max-min+1.0)/(1.0+RAND_MAX));
}
...
/*
float ChaosRandom(float min, float max)
{
    float x1[2];
    x1[0] = min;
    x1[1] = max;
    ...
    for(int ch=0;ch<2;ch++) {
        ch_x = x1[ch];
        if(ch_x<0.0) {
            ch_x = ch_x + 2 * ch_x * ch_x ;
        } else {
            ch_x = ch_x - 2 * (1.0 - ch_x) * (1.0 - ch_x);
        }
        if (ch_x<0.005) {
            ch_x = (float)(rand())/256;
        }
...
static void draw_ground2() {
    glBegin(GL_QUADS);
    glColor3f(0.0, 0.0, 0.0);
    float array_right[draw_flg+1];
    float array_left[draw_flg+1];
    
    for (int n=0; n<draw_flg; n++) {
        array_right[n]=copy_array_right[n];
        array_left[n]=copy_array_left[n];
    }

    /*
    float x1[2];
    x1[0] = GetRandom(0.0, 1.0);
    x1[1] = GetRandom(0.0, 1.0);
    float ch_x;
    for(int ch=0;ch<2;ch++) {
        ch_x = x1[ch];
        if(ch_x<0.5) {
...
//        ch_x = ch_x * 251.0 + 5.0;
    ...
    array_right[draw_flg]=GetRandom_right(-3.0, 3.0);
    array_left[draw_flg]=GetRandom_left(-3.0, 3.0);
//    array_right[draw_flg]=x1[0];
//    array_left[draw_flg]=x1[1];
    
    for (int i =0; i<draw_flg+1; i++) {
        glVertex2d(gr_xpos + 12*i/*gr_count*/, (ysize - gr_ypos) - (ysize / 12)+array_right[i]); // KOKO! (BUFFALO = 12)(CCD = 15)
        glVertex2d(gr_xpos + 12*(i+1)/*gr_count*/, (ysize - gr_ypos) - (ysize / 12)+array_right[i+1]);
        glVertex2d(gr_xpos + 12*(i+1)/*gr_count*/, 0);
        glVertex2d(gr_xpos + 12*i/*gr_count*/, 0);
        
        glVertex2d(gr_xpos - 12*i/*gr_count*/, (ysize - gr_ypos) - (ysize / 12)+array_left[i]); // KOKO! (BUFFALO = 12)(CCD = 15)
        glVertex2d(gr_xpos - 12*(i+1)/*gr_count*/, (ysize - gr_ypos) - (ysize / 12)+array_left[i+1]);
        glVertex2d(gr_xpos - 12*(i+1)/*gr_count*/, 0);
        glVertex2d(gr_xpos - 12*i/*gr_count*/, 0);
    }
    
    for (int m=0; m<draw_flg+1; m++) {
        copy_array_right[m]=array_right[m];
        copy_array_left[m]=array_left[m];
    }
